package com.arielbg.crudapi.controller;

import com.arielbg.crudapi.model.Producto;
import com.arielbg.crudapi.repository.ProductoRepository;
...
public class ProductoRestController {
    ...
    public ResponseEntity<?> agregarProducto(@RequestBody Producto p) {
        if (p.getPrecioUnitario().compareTo(BigDecimal.ZERO) <= 0 || p.getCantidadStock() < 0) {
            return ResponseEntity.badRequest().body("El Precio debe ser mayor a 0 y la cantidad no puede ser negativa.");
        }

        Optional<Producto> productoExistente = repository.findById(p.getCodigo());
        if (productoExistente.isPresent()) {
            return ResponseEntity.status(HttpStatus.CONFLICT)
                    .body("Ya existe un producto con el código número: " + p.getCodigo());
        }
    ...
    public ResponseEntity<?> obtenerProducto(@PathVariable Integer codigo) {
        ...
        if (producto.isPresent()) {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("No se encontró el producto con el código número: " + codigo);
        }
    }
    ...
    public ResponseEntity<?> eliminarProducto(@PathVariable Integer id) {
        ...
        if (producto.isPresent()) {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("No se encontró el producto con el código número: " + id);
        }
    }
    ...
    public ResponseEntity<?> obtenerEstadisticas() {
        ...
        if (productos.isEmpty()) {
            return ResponseEntity.status(HttpStatus.NO_CONTENT).body("No hay productos.");
        }
